...
date: "Due: Friday, September 4, 2015"
output:
  html_document:
    highlight: pygments
    theme: cerulean
...
#read.table produces fewer observations (1698, fails)
gapminder2 <- read.table("http://www.stat.ubc.ca/~jenny/notOcto/STAT545A/examples/gapminder/data/gapminderDataFiveYear.txt",header =TRUE) 
```

  * If you're not getting the same object, why not?
Read..table tells us that there is an error in line 145. This is related to the overall combined length
...
```{r}
read.table("http://www.stat.ubc.ca/~jenny/notOcto/STAT545A/examples/gapminder/data/gapminderDataFiveYear.txt",header =TRUE)

gapminder2 <- read.table("http://www.stat.ubc.ca/~jenny/notOcto/STAT545A/examples/gapminder/data/gapminderDataFiveYear.txt",header =TRUE,sep='\t') 
```
